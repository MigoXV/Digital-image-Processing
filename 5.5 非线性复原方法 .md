...
$$
\max _{f} p(f \mid g)=\max _f p(g \mid f) p(f) / p(g)=\max _f p(g \mid f) p(f)
$$
...
$f^{(1)}$ 取 $f^{(0)}$ 在第一个超平面
$$
a_{11} f_1+a_{12} f_2+\ldots \quad a_{1 N} f_{N}=g_1 \text { 上的投影。 }
$$
即: $f^{(1)}=f^{(0)}-\frac{f^{(0)} \cdot a_1-g_1}{a_1 \bullet a_2} a_1$
(5. 49)
$f^{(2)}$ 取 $f^{(1)}$ 在第二个超平面
$a_{21} f_1+a_{22} f_2+\ldots \quad a_{2 N} f_{N}=g_2$ 上的投影。

依次继续下去, 直到满足 $( 5 . 4 8 )$ 式中最后一个方程式。这就实现了迭代的第一个循环然后再从 (5.48)式中第一个方程式开始第二次迭代。即取 $f^{(M)}$ 在第一个超平面
$$
a_{11} f_1+a_{12} f_2+\ldots a_{1 N} f_{N}=g_1
$$
上的投影。并称之为 $f^{(M+1)}$ 。依次继续, 直到
$$
f^{(k+1)}=f^{(k)}
$$
则: $f^{(k)}=f_{\text {。 }}$
