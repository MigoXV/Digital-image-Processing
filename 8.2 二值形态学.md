...
如果 $S$ 包含了原点, 即 $\mathrm{O} \in S$, 那么 $X \ominus S$ 将是 $X$ 的一个收缩,即 $X \ominus S \subseteq X$ (当 $\mathrm{O} \in S$ 时）; 如果 $S$ 不包含原点, 那么X $X \subseteq X$ 末必成立。如果结构元素 $S$ 关于原点 $\mathrm{O}$ 是对称的, 那么 $S=S^{\vee}$, 因此 $X$$\ominus S=X \ominus S^{\mathrm{v}}$, 但是, 如果 $S$ 关于原点 $\mathrm{O}$ 不是对称的, 那么 $X$ 被 $S$ 腐蚀的结果与 $X$ 被 $S^{\mathrm{V}}$ 腐蚀的结果是不同的。
...
也就是说, 由 $S$ 对 $X$ 膨胀所产生的二值图象满足以下条件的点 $(x, y)$ 的集合：如果 $\mathrm{B}$的原点平移到点 $(x, y)$, 那么它与 $X$ 的交集非空。 与式（8-4）等价的膨胀运算定义形式还有:
(1) 
...
式 (8-12) 表明, $X$ 被 $S$ 击中的结果相当于 $X$ 被 $S_{1}$ 腐蚀的结果与$X$ 被 $S_{2}$ 的反射集 $S_{2} \mathrm{~V}$ 膨胀的结果之差。图8-15解释了这一过程。由 此可见, 击中运算也可以借助于腐蚀、膨胀两基本运算来实现。 
